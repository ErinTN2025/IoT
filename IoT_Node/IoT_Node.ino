...
const char* ssid = "AT";
const char* password = "Dhirawatm170";
const char* mqtt_server = "192.168.50.103";
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (LED == 1) {
    ...
    Serial.println("Turn ON the warning light");
  } else {
    digitalWrite(BUILTIN_LED, LOW);  // Turn the LED off by making the voltage LOW
    Serial.println("Turn OFF the warning light");
  }
}
...
void loop() {
  ...
  if (now - lastMsg > 2000) {
    ...
    int sensorData = random(1, 1023);
    doc["sensor"] = "Gas sensor";
    doc["value"] = sensorData;
    char jsonStr[60];
    serializeJson(doc, jsonStr);
    Serial.println();
    client.publish("sensingData", jsonStr);
  }
}
